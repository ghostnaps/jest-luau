...
describe(".toBe()", function()
	it("does not throw", function()
		...
		-- jestExpect(BigInt(1)).never.toBe(BigInt(2))
		-- jestExpect(BigInt(1)).never.toBe(1)
			...
			original code:
			  [
			    ...
			    it(`fails for: ${stringify(a)} and ${stringify(b)}`, () => {
			      expect(() => expect(a).toBe(b)).toThrowError('toBe');
			    });
			  });
		]]
			...
			original code:
			  [BigInt(1), BigInt('1')].forEach(v => {
			    it(`fails for '${stringify(v)}' with '.not'`, () => {
			      expect(() => expect(v).not.toBe(v)).toThrowError('toBe');
			    });
			  });
		]]

	-- ROBLOX deviation START: additional test - we can't test nil as part of the loop above because the for loop
	...
	it.skip("assertion error matcherResult property contains matcher name, expected and actual values", function()
		...
		if not ok then
			expect(error_.message).toEqual(expect.objectContaining({
				actual = actual,
				expected = expected,
				name = "toBe",
			}))
...
describe(".toStrictEqual()", function()
	...
	-- 	expect({, , , 1, , ,}).never.toStrictEqual({, , , 1, nil, ,});
	-- end)
	-- ROBLOX deviation END
	...
	it("does not simply compare constructor names", function()
		...
		expect(tostring(c)).toEqual(tostring(d))
		expect({ test = c }).never.toStrictEqual({ test = d })
	end)
	...
	it.skip("does not pass when sparseness of arrays do not match", function()
		-- expect({, 1}).never.toStrictEqual({nil, 1})
		-- expect({nil, 1}).never.toStrictEqual({, 1})
		-- expect({, , , 1}).never.toStrictEqual({, 1})
	end)

	it.skip("does not pass when equally sparse arrays have different values", function()
		-- expect({, 1}).never.toStrictEqual({, 2})
	end)
...
describe(".toEqual()", function()
	...
	it("assertion error matcherResult property contains matcher name, expected and actual values", function()
		...
		if not ok then
			expect(error_.matcherResult).toEqual(expect.objectContaining({
				actual = actual,
				expected = expected,
				name = "toEqual",
			}))
	...
	it("symbol based keys in arrays are processed correctly", function()
		...
		expect(actual2).never.toEqual(expected)
	end)
...
describe(".toBeInstanceOf()", function()
	...
	it("failing A.new() and B", function()
		...
		expect(A.new()).never.toBeInstanceOf(B)
	end)
end)
...
describe(".toBeNan()", function()
	...
	-- ROBLOX deviation START: add tests for our alias
...
describe(".toBeDefined() .toBeUndefined()", function()
	for _, testCase in ipairs({
		...
		it(stringify(testCase) .. " is defined", function()
			...
			jestExpect(testCase).never.toBeUndefined()
...
describe(".toMatch()", function()
	...
	for _, testCase in ipairs({
		...
		local n2 = testCase[2]
		it(
			string.format("throws if non String/RegExp expected value passed: [%s, %s]", stringify(n1), stringify(n2)),
			function()
				expect(function()
					jestExpect(n1).toMatch(n2)
				end).toThrowErrorMatchingSnapshot()
...
describe(".toHaveLength", function()
	...
	-- ROBLOX deviation START: add custom test to allow for Lua objects with a length value
	local obj = { length = 12 }
...
describe(".toHaveProperty()", function()
	...
	for _, testCase in ipairs({
		...
		{ { ["a.b.c.d"] = 1 }, { "a.b.c.d" }, 1 },
		{ { a = { b = { 1, 2, 3 } } }, { "a", "b", 2 }, 2 },
		{ { a = { b = { 1, 2, 3 } } }, { "a", "b", 2 }, expect.any("number") },
		{ { a = 0 }, "a", 0 },
		{ { a = { b = false } }, "a.b", false },
		--[[
	...
	for _, testCase in ipairs({
		...
		it(string.format("{pass: true} expect(%s).toHaveProperty(%s)", stringify(obj), stringify(keyPath)), function()
